...
const ProjectInfoBar_v2 = ({
  ...
  const isProjectFavorite = userInfo.favProjectIdList
    .map((project) => project._id)
    .includes(projectInfo._id);
  ...
  const handleFavoriteClick = async (e) => {
    ...
    if (isStarred) {
      ...
      const response = await fetch(
        `${backendUrl}/user/removeProjectFromFavorite`,
        {
          method: "PUT",
          headers: {
      ...
      const newFavProjectIdList = [
        ...userInfo.favProjectIdList,
        projectInfo._id,
      ];
      // setUserInfo({ ...userInfo, favProjectIdList: newFavProjectIdList });

      const response = await fetch(`${backendUrl}/user/addProjectToFavorite`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        ...
        <Container className={classes.mainSection} fluid={true} mb={"0px"}>
          <Group position="apart">
            <Group>
              ...
              <Group>
                <IconBook2 size={70} />
                <Title order={3}>{projectInfo.projectName}</Title>
                {isStarred && (
                  <IconStarFilled
                    size={20}
                    onClick={(e) => {
