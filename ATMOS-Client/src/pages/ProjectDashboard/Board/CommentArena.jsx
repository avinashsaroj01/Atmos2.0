...
const CommentArena = ({ taskInfo, taskComments, setTaskComments, userInfo }) => {
  ...
  const commentHandle = async (e) => {
    ...
    console.log(taskInfo);
    const saveMsg = await fetch(`${backendUrl}/taskList/${taskInfo.id}`, {
      method: "PUT",
    ...
    <div className={styles.commentArena}>
      <div className={styles.commentList}>
        {taskComments.length === 0 && (
          <h1 className={styles.noCommentHeading}>No Comment Yet</h1>
        )}
        {taskComments &&
      ...
      <div className={styles.writingArena}>
        ...
        <div className={styles.commentButton}>
          <button className={styles.commentButtonBtn} onClick={commentHandle}>
            Comment
          </button>
        </div>
      </div>
