...
const SectionArena = ({ projectId, projectInfo, setProjectInfo, userInfo, setUserInfo }) => {
  ...
  const [sectionList, setSectionList] = useState(
    projectInfo.projectSectionIdList,
  );
  ...
  const createSection = async () => {
    const sec = await fetch(`${backendUrl}/section/create`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  for (let i = 0; i < projectInfo.projectHighAccessMembers.length; i++) {
    AssigneeList.push({
      value: projectInfo.projectHighAccessMembers[i]._id,
      label: projectInfo.projectHighAccessMembers[i].userName,
    });
  }
  for (let i = 0; i < projectInfo.projectMediumAccessMembers.length; i++) {
    AssigneeList.push({
      value: projectInfo.projectMediumAccessMembers[i]._id,
      label: projectInfo.projectMediumAccessMembers[i].userName,
    });
  }
  for (let i = 0; i < projectInfo.projectLowAccessMembers.length; i++) {
    AssigneeList.push({
      value: projectInfo.projectLowAccessMembers[i]._id,
      label: projectInfo.projectLowAccessMembers[i].userName,
    });
  }
      ...
      <div className="section-arena-container">
        ...
        >
          ...
          >
            {(provided) => (
              <div
                className="section-arena"
                {...provided.droppableProps}
                ref={provided.innerRef}
              >
                {columnOrder &&
                  columnOrder.map((columnIndex, index) => (
                    <Draggable
                      key={columnIndex}
        ...
        {(userAccessLevel === "owner" || userAccessLevel === "high") && (
          <div className="section-arena">
            <Button w={315} color="blue" onClick={createSection}>
              Add Section
            </Button>
          </div>
        )}
