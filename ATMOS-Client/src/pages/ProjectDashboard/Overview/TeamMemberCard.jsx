...
const TeamMemberCard = ({ name, email, role, id, projectOwnerId, projectId, userAccessLevel, avatar, rightSectionIcon = false, rerender, setRerender }) => {
      ...
      children: (
        <div>
          ...
          <Text color="dimmed" size="sm">
            This will change the access level of{" "}
            <span className="text-primary fw-bolder">{name}</span> to{" "}
            <span className="text-primary fw-bold">
              {memberRole === "highAccess"
                ? "High"
                : memberRole === "mediumAccess"
                  ? "Medium"
                  : "Low"}
            </span>
          </Text>
  ...
  const handleAccessChange = async () => {
    try {
      ...
      const res = await fetch(
        `${backendUrl}/project/${projectId}/changeUserAccessLevel`,
        {
          method: "POST",
          headers: {
    ...
    openConfirmModal({
      ...
      children: (
        <div className="">
          ...
          <Text color="dimmed" size="sm">
            This will remove <span className="text-danger fw-bold">{name}</span>{" "}
            from the team
          </Text>
        </div>
      ),
  ...
  const handleRemoveMember = async () => {
    try {
      ...
      const res = await fetch(
        `${backendUrl}/project/${projectId}/removeTeamMember`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
